...
export const signsList: SignData[] = [
  ...
  { id: "no", english: "No", arabic: "لا", description: "Shake index finger side to side" },
...
export function NoSign({ isHighlighted }: { isHighlighted?: boolean }) {
  return (
    <motion.div
      animate={isHighlighted ? { rotate: [0, -15, 15, -15, 0] } : {}}
      transition={{ duration: 0.8, ease: "easeInOut", repeat: isHighlighted ? Infinity : 0, repeatDelay: 1.5 }}
      className="text-primary w-24 h-24"
...
export function AnimatedSign({ signId, isHighlighted }: { signId: string, isHighlighted?: boolean }) {
  switch (signId) {
    ...
    case "no": return <NoSign isHighlighted={isHighlighted} />;
